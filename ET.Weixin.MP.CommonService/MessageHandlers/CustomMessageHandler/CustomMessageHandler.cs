...
using ET.Weixin.MP.CommonService.Utilities;
using System.Collections.Generic;
using Senparc.Weixin.MP;

namespace ET.Weixin.MP.CommonService.CustomMessageHandler
{
    /// <summary>
    /// 自定义MessageHandler
    ...
    public partial class CustomMessageHandler : MessageHandler<CustomMessageContext>
    {
...
#if DEBUG
        string agentUrl = "http://localhost:8730/Weixin";
        string agentToken = "etmanage";
        ...
        public override IResponseMessageBase OnTextRequest(RequestMessageText requestMessage)
        {
            ...
            switch (requestMessage.Content)
            {
                ...
                case "文章":
                case "关于我们":
                      var strongResponseMessage1 = CreateResponseMessage<ResponseMessageNews>();
                      responseMessage = strongResponseMessage1;
                      strongResponseMessage1.Articles.Add(new Article()
                        {
                ...
                default:
                    ...
                    if (!string.IsNullOrEmpty(requestMessage.Content))
                        strCondition = " AND charindex('" + requestMessage.Content + "', ArticleTitle)>0";
                    List<ET.Sys_DEF.BlogArticleInfo> listArticle = new ET.Sys_BLL.PublicBLL().GetListByCondition<ET.Sys_DEF.BlogArticleInfo>(3, "ArticleID,ArticleTitle,ArticleLabel,ArticleDescription,ArticleCover,CreateTime,AccessCount,LoveCount,ShareCount,ArticleUrl,TypeID,(select count(1) from BlogCommentInfo where BlogCommentInfo.ArticleID=BlogArticleInfo.ArticleID) ArticleSource", ET.Constant.DBConst.TableNames.BlogArticleInfo, "AND Status=1 " + strCondition, "CreateTime DESC");
                    var strongResponseMessage2 = CreateResponseMessage<ResponseMessageNews>();
                    responseMessage = strongResponseMessage2;
                    foreach (ET.Sys_DEF.BlogArticleInfo info in listArticle)
                    {
                        strongResponseMessage2.Articles.Add(new Article()
                        {
